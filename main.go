...
//go:generate go run github.com/cilium/ebpf/cmd/bpf2go -no-global-types -cc clang xdpfn ./ebpf/xdp.c -- -D__TARGET_ARCH_x86 -I./ebpf/headers -Wall

func main() {
	var dev string
	var latency string
	flag.StringVarP(&dev, "dev", "D", "", "device to inject latency to ping")
	flag.StringVarP(&latency, "lat", "L", "1ms", "latency to delay, 1ms <= latency <= 10s")
	flag.Parse()

	dur, err := time.ParseDuration(latency)
	if err != nil {
		log.Fatalf("Failed to parse latency(%s): %v", latency, err)
	}

	if 1*time.Millisecond > dur || dur > 10*time.Second {
		log.Fatalf("Latency should be 1ms <= latency <= 10s")
	}
	...
	for {
		...
		if nRecv != 0 && !isExiting {
			...
			delayPackets(xsk, descs, dur)
		}
	}
}

func delayPackets(xsk *xdp.Socket, descs []xdp.Desc, latency time.Duration) {
	_ = time.AfterFunc(latency, func() {
		transformPackets(xsk, descs)
		_ = xsk.Transmit(descs)
	})
}

func transformPackets(xsk *xdp.Socket, descs []xdp.Desc) {
